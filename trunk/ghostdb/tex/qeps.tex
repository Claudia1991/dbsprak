...
Von den Piloten aus wird analog der Weg über Buchungen beschritten. ``Climbing Index'': \{\{\}, \{B08, B11, B18, B19, B20, B21\}, \{\}, \{\}\}, Merge: \{B08, B11, B18, B19, B20, B21\}.
...
Mittels MJoin werden die noch verbleibenden Projektionen ausgeführt: \\
 \{$\langle$R04,Doru,Davidovici,Romänisch$\rangle$\}.
...
Das unsichere Gerät liefert die Gepäckstücke, die schwerer als 20 Kilogramm sind. Es wir dein Bloom-Filter erzeugt und auf das vorhandene Zwischenergebnis angewendet: \{$\langle$G07,R11$\rangle$, $\langle$G18,R07$\rangle$, $\langle$G19,R04$\rangle$, $\langle$G20,R04$\rangle$, $\langle$G22,R02$\rangle$, $\langle$G23,R01$\rangle$\}. Da die \textit{GepaeckIDs} nicht mehr benötigt werden, werden nur die \textit{ReisendeIDs} aufgehoben: \{R07, R04, R04, R02, R01\}
...
Mittels ``Climbing Index'' ist der Schritt von \textit{Pilot.Geburtsdatum} auf \textit{BuchungsID} möglich: \{\{\}, \{B08, B11, B18, B19, B20, B21\}, \{\}, \{\}\}. Durch Verwendung von Merge und eines ``Subtree Key Tables'' wird eine Liste von \textit{ReisendeIDs} berechnet: \{R07, R03, R13, R05, R04, R11\}.
...
Aus dem Durchschnitt beider Lsiten von \textit{ReisendeIDs} wird ein Bloom-Filter erzeugt. Dieser wird auf das übermittelte Zwischenergebnis des unsicheren Geräts (Reisender.Geschlecht='M') angewendet. Durch Verwendung des MJoin-Operators wird das Ergebnis erzeugt:\\
 \{$\langle$R04,Doru,Davidovici,Romänisch$\rangle$\}.
