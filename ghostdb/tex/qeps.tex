...
Von \textit{Piloten} aus wird analog der Weg über \textit{Buchungen} beschritten. ``Climbing Index'': \{\{\}, \{B08, B11, B18, B19, B20, B21\}, \{\}, \{\}\}, Merge: \{B08, B11, B18, B19, B20, B21\}.
...
Unter Verwendung der in \cite{ghostdb1} definierten Operanden, lässt sich der Prefiltering Plan auch wie folgt aufschreiben:
...
Das Problem an dieser Strategie ist allerdings, dass bei zu geringer Selektivität der sichtbaren Selektionen zu wenige Tupel im Vorfeld aussortiert werden und somit ein großer Vorteil des Prefilterings verschwindet. Eine Alternative, die diesem Effekt entgegen wirkt ist das Postfiltering.

\section{Postfiltering}
Beim Postfiltering werden zuerst alle Selektionen ausgeführt und mittels Joins zusammengefasst, die auf den sicheren Bereich zugreifen. Erst danach werden unter Verwendung von Bloom-Filtern die Selektionen des unsicheren Bereiches hinzugefügt.

Für unser Beispiel heißt dies, dass zuerst die Selektion auf dem Geburtsdatum des Piloten ausgeführt wird. Es wird der ``Climbing Index'' verwendet, um \textit{GepackIDs} zu erhalten: \{\{\}, \{G07, G18, G19, G20, G25\}, \{\}, \{\}\}.
...
Anschließend wird ein ``Subtree Key Table'' genutzt, um passende \textit{ReisenderIDs} zu berechnen: \{$\langle$G07,R11$\rangle$, $\langle$G18,R07$\rangle$, $\langle$G19,R04$\rangle$, $\langle$G20,R04$\rangle$, $\langle$G22,R02$\rangle$, $\langle$G23,R01$\rangle$\}
...
Das unsichere Gerät liefert die Gepäckstücke, die schwerer als 20 Kilogramm sind. Es wird ein Bloom-Filter erzeugt und auf das vorhandene Zwischenergebnis angewendet: \{$\langle$G07,R11$\rangle$, $\langle$G18,R07$\rangle$, $\langle$G19,R04$\rangle$, $\langle$G20,R04$\rangle$, $\langle$G22,R02$\rangle$, $\langle$G23,R01$\rangle$\}. Da die \textit{GepaeckIDs} nicht mehr benötigt werden, werden nur die \textit{ReisenderIDs} aufgehoben: \{R07, R04, R04, R02, R01\}
...
Mittels ``Climbing Index'' ist der Schritt von \textit{Pilot.Geburtsdatum} auf \textit{BuchungsID} möglich: \{\{\}, \{B08, B11, B18, B19, B20, B21\}, \{\}, \{\}\}. Durch Verwendung von Merge und eines ``Subtree Key Tables'' wird eine Liste von \textit{ReisenderIDs} berechnet: \{R07, R03, R13, R05, R04, R11\}.
...
Aus dem Durchschnitt beider Listen von \textit{ReisenderIDs} wird ein Bloom-Filter erzeugt. Dieser wird auf das übermittelte Zwischenergebnis des unsicheren Geräts, Reisender.Geschlecht='M', angewendet. Durch Verwendung des MJoin-Operators wird das Ergebnis erzeugt: \\\{$\langle$R04,Doru,Davidovici,Romänisch$\rangle$\}.
...
Fügt man alle Teile zusammen, erhält man den kompletten Ablaufplan für unsere Anfrage:
...
Unter Verwendung der in \cite{ghostdb1} definierten Operanden, lässt sich der Postfiltering Plan auch wie folgt aufschreiben:
