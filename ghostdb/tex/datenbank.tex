In den Papern zu diesem Thema wurden meist Beispiele aus dem medizinischen Bereich mit Ärzten, Patienten, Untersuchungen und mehr gewählt. Wir haben uns für ein anderes Feld entschieden: den Flugverkehr. Hier existieren viele verschiedene Daten, die je nach Angreifermodell und Nutzer zu schützen sind. Eine Fluggesellschaft ist zum Beispiel daran interessiert, welche Reisende ihr Kontingent an Gepäckstücken beziehungsweise Gepäckgewicht überschreiten, um ihnen beim nächsten Flug ein teureres Ticket verkaufen zu können. Andererseits soll aber nicht für jeden erkennbar sein wer, wann, wohin geflogen ist.

Die von uns erdachte Datenbank besteht aus 6 Tabellen: Buchungen, Gepaeck, Fluege, Reisende, Flugzeuge und Piloten. Die jeweils enthaltenen Attribute sind in den Tabellen \ref{tab:SchemaBuchungen}-\ref{tab:SchemaPiloten} zu sehen. Grau hinterlegte Attribute, häufig Fremdschlüssel, müssen unserer Meinung nach besonders geschützt werden und werden deshalb auf dem ``SmartUSB''-Stick gespeichert.
...
Um die verwendeten Strukturen, ``Subtree Key Table'' und ``Climbing Index'', adäquat vorzustellen, haben wir die ausgedachten Tabellen mit Daten ausgefüllt. Dies ist in den Tabllen \ref{tab:Buchungen} bis \ref{tab:Piloten} dargestellt. Einige Tabellen enthalten Attribute, die nicht frei erfunden sind, insbesondere Personen und Orte. Die verwendeten Quellen dazu sind im Anhang aufgelistet.
